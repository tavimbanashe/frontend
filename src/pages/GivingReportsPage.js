...
const GivingReportsPage = () => {
    ...
    const [isAdding, setIsAdding] = useState(false);  // Managing visibility of modal
    ...
    const [selectedMemberId, setSelectedMemberId] = useState(''); // State for selected member ID
    ...
    const handleAddGivingSubmit = async (e) => {
        ...
        try {
            const response = await fetch('/api/giving-reports', {
                ...
                body: JSON.stringify({
                    member_id: selectedMemberId,  // Save member ID instead of name
            ...
            if (response.ok) {
                ...
                setSelectedMemberId('');  // Clear selected member
            } else {
    ...
    // Calculate max value for the Y-Axis to dynamically adjust it
    ...
    return (
        <div className="dashboard">
            ...
            <main className="giving-reports-page">
                ...
                {/* Modal for Adding New Giving Entry */}
                {isAdding && (
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                        <BarChart data={summary}>
                            ...
                            {summary.map((data, index) => (
                                <Bar key={data.type} dataKey="total_amount" fill={COLORS[index % COLORS.length]} />
                            ))}
                        </BarChart>
                    </ResponsiveContainer>
                </section>

...
const MembersPage = () => {
    ...
    useEffect(() => {
        setLoading(true);
        fetchData(`${process.env.REACT_APP_API_BASE_URL}/api/members`, setMembers, 'Error fetching members');
        fetchData(`${process.env.REACT_APP_API_BASE_URL}/api/members/dropdowns`, setDropdownData, 'Error fetching dropdown data');
        setLoading(false);
    }, []);
    ...
    const handleDeleteMember = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/members/${memberToDelete}`, {
                method: 'DELETE',
            });
            if (!response.ok) {

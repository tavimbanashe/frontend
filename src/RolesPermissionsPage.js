...
const RolesPermissionsPage = () => {
    ...
    const fetchPermissions = async () => {
        try {
            const response = await fetch('/api/roles-permissions/permissions');
            const data = await response.json();
            setPermissions(data.map((permission) => ({ ...permission, id: permission.permission_id })));
        } catch (error) {
            ...
            <main className="roles-permissions-page">
                ...
                <section className="permissions-grid">
                    ...
                    <div style={{ height: 400, width: '100%' }}>
                        <DataGrid
                            rows={permissions}
                            columns={[{ field: 'permission_name', headerName: 'Permission Name', width: 250 }]}
                            pageSize={5}
                            rowsPerPageOptions={[5, 10, 20]}
                        />
                    </div>
